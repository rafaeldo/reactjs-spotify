...
export default function player(state = INITIAL_STATE, action) {
  switch (action.type) {
    ...
    case Types.NEXT:
      const nextCurrentIndex = state.list.findIndex(song => song.id === state.currentSong.id);
      const next = state.list[nextCurrentIndex + 1];
      ...
      return state;
    case Types.PREV:
      const prevCurrentIndex = state.list.findIndex(song => song.id === state.currentSong.id);
      const prev = state.list[prevCurrentIndex - 1];
